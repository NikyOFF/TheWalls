        ...
        if (args[0].equals(MapCommandActions.add.name())) {
            ...
            if (args[1].equals(MapCommandSettingsActions.barColor.name())) {
                ...
                if (args.length == 2 && sender instanceof Player) {
                    ...
                    Main.Singleton.MapManager.CurrentMap.WorldBorderCanterLocation = new Location(((Player) sender).getWorld(), Double.parseDouble(args[2]), Double.parseDouble(args[3]), Double.parseDouble(args[4]));
                    return true;
                } else if (args.length == 6) {
                    World world = Bukkit.getWorld(args[2]);

                    if (world != null) {
                        Main.Singleton.MapManager.CurrentMap.WorldBorderCanterLocation = new Location(world, Double.parseDouble(args[3]), Double.parseDouble(args[4]), Double.parseDouble(args[5]));
                        return true;
                    }
                }
